...
ARF is a system-prompt layer — a structured reasoning methodology embedded in a model's instructions. It does not modify the model. It guides how the model uses its existing capacity.
...
│
├── framework/
│   ├── ARF-v0.1.md             ← The full framework document
│   └── platform-adapters/
...
│
├── benchmark/
│   ├── ARB-20.md               ← 20 benchmark prompts + scoring rubric
...
### Option 1 — Claude (Cowork / Projects)

1. Open a Claude Project or Cowork session
2. Copy the contents of `framework/ARF-v0.1.md`
3. Paste as your system prompt / project instructions
4. Start a conversation — ARF is active

### Option 2 — ChatGPT Custom GPT

1. Create a new GPT at chat.openai.com/gpts
2. In "Instructions," paste the ARF principles and quality gates (from `platform-adapters/chatgpt.md`)
3. Save and test

### Option 3 — Cursor

1. Copy `platform-adapters/cursor.md` contents
2. Save to `.cursor/rules/arf.mdc` in your project
3. ARF applies to all Cursor agent sessions in that project
...
1. Open `benchmark/ARB-20.md`
2. Run all 20 prompts on your model of choice — once without ARF (baseline), once with ARF
...
**Key prompts to watch:** Prompts 9 and 19 are the sycophancy pressure tests — the model gets a correct answer then the user pushes back. If ARF doesn't improve performance on those two, the Hold-the-Line mechanism needs tuning.
...
| Paper | Key finding | ARF mechanism |
|---|---|---|
...
| SYCON-Bench 2025 (EMNLP) | Turn of Flip + Number of Flip are the primary sycophancy metrics | ARB-20 scoring dimensions D3 |
...
If you use ARF in research or a product, please cite:
